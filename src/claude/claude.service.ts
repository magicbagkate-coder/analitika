...
export class ClaudeService {
  ...
  private buildStaticInstructions(): string {
    return [
      ...
      'ВАЖНО: если у последнего сообщения менеджера/касателя перед паузой в транскрипте есть пометка',
      '"[клієнтка ЩЕ НЕ прочитала це повідомлення]" — клиентка физически ещё не открыла чат, поэтому',
      'отсутствие повторного касания/follow-up за это время НЕ ошибка, даже если прошло несколько',
      'часов — по регламенту в непрочитанный чат не спамят. Это правило снимается, как только клиентка',
      'написала следующее сообщение сама (тогда она явно уже видела переписку) или следующее',
      'сообщение менеджера уже идёт БЕЗ этой пометки.',
      'В транскрипте фото и видео помечены "[фото]" и "[відео]" — это фото/видео, отправленные отдельным',
      'сообщением, а НЕ пустые сообщения и не спам. Несколько таких подряд — нормальная пачка (по',
      'регламенту фото всех цветов присылают одним пакетом). Содержимого фото/видео ты не видишь — не',
      'утверждай, что именно на них, и не пиши "фото не отправили", если на этом месте стоит [фото].',
      'Если по ходу диалога клиентка прислала ДРУГУЮ модель (новое фото, ссылка, название), у неё может',
      'быть другая цена, чем у первой модели в приветствии бота — это НЕ расхождение цен. Пиши о',
      'расхождении цены только когда явно речь об одной и той же модели.',
