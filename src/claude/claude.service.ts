...
export class ClaudeService {
  ...
  private buildStaticInstructions(): string {
    return [
      ...
      '"Старший менеджер" — от него особенно ждут допродаж (второй товар, аксессуар), а не только',
      'консультации по основному товару.',
      '"Керівник зміни" — если клиентка дала сигнал "важного" лида (сказала "подумаю", "порадитись",',
      'или это подарок) и после этого сигнала НИКТО (ни керівник, ни касатель) не сделал повторного',
      'персонального касания — это реальный процессный пробел, стоит прямо об этом написать.',
  ...
  private buildEvaluationTool(): Anthropic.Tool {
    ...
    return {
      ...
      input_schema: {
        ...
        required: ['score', 'purchased', 'goodPoints', 'closingSummary', 'mistakes', 'recommendation'],
      },
    };
  }
