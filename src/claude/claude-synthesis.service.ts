...
export class ClaudeSynthesisService {
  ...
  private buildSynthesisPrompt(outcomes: PatternSynthesisInput[]): string {
    ...
    return [
      ...
      'Допродажа уместна только ПОСЛЕ отправки реквізитів на оплату — не считай паттерном отсутствие',
      'допродажи на более раннем этапе. Повторное касание после "подумаю"/"напишу вам" — обязанность',
      'только касателя, не старшого менеджера и не керівника зміни — не включай это в паттерн для них.',
      '',
