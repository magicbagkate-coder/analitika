...
import { ANALYSIS_WINDOW_HOURS, TARGET_STATUS, filterToRecentWindow } from '../src/evaluation/evaluation.constants';
import { SitniksChatListService } from '../src/sitniks-chat-list/sitniks-chat-list.service';
import { SitniksChatMessagesService } from '../src/sitniks-chat-messages/sitniks-chat-messages.service';
import { formatChatBlock, formatSummaryBlock } from '../src/status-report/status-report-formatter';
import { TelegramService } from '../src/telegram/telegram.service';
import type { ChatListItem } from '../src/sitniks-chat-list/sitniks-chat-list.types';
import type { ChatMessage } from '../src/sitniks-chat-messages/sitniks-chat-messages.types';
...
function collectManagerNames(messages: ChatMessage[]): string[] {
...
function isLost(messages: ChatMessage[]): boolean {
...
async function fetchAllChatsInStatus(sitniksChatListService: SitniksChatListService): Promise<ChatListItem[]> {
  const all: ChatListItem[] = [];
  let skip = 0;
  const pageSize = 50;

  for (;;) {
    const response = await sitniksChatListService.listChats({ status: TARGET_STATUS, skip, limit: pageSize });
...
async function main(): Promise<void> {
  ...
  const sitniksChatListService = app.get(SitniksChatListService);
  ...
  if (!isTestRun) {
    await telegramService.sendMessage(
      `⚡<b>Пересчёт оценок отчёта 15:45 — обновлена калибровка:</b> оценка теперь отражает качество работы ` +
  ...
  for (const chatId of targetIds) {
    try {
      ...
      if (!chat) {
        console.log(`${chatId}: no longer in "${TARGET_STATUS}", skipped (status likely changed since 15:45)`);
        continue;
      }
      ...
      if (oldScoreTag !== newScoreTag) changed += 1;
      ...
      if (!isTestRun) {
        await telegramService.sendMessage(formatChatBlock(outcome));
      }
    } catch (error) {
      console.error(`${chatId}: FAILED - ${(error as Error).message}`);
    }
    await delay(DELAY_MS);
  }
  ...
  await telegramService.sendMessage(formatSummaryBlock(outcomes.length, outcomes, { critical, patterns: draft.patterns }));
